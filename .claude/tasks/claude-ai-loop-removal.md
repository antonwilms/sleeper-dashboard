...
**Pass 1 — every app-side data-store fetch/validate call site.** `grep -rn "tryDataStore(" src/` (excluding tests) returns 11 non-test call sites; each maps to a family:
...
#### 2.2.0 The two completeness burdens are not the same — and this is what closes the review loop

The three rounds of app-side trigger gaps were treated as a defect to be ground out by ever-more-thorough grepping. That framing is wrong, and chasing it is unbounded: `src/` changes every week, so any static list of app-side consumers is stale the moment a new module reads a served field. The registry does not actually need that list to be provably exhaustive — because of *who reads it when*:

| Side of `‖` | Burden | Why |
|---|---|---|
| **Data side** (`lib/`, `scripts/`, `bin/`, README, mirrored constants) | **Frozen-complete.** Must be right in the registry, verified once, kept right by the both-repos-same-change rule. | After the Claude.ai loop is removed, the app-repo subagent checks cross-repo intent **against the registry and never reads the sibling tree** (`tools: Read, Grep, Glob`, no sibling access). A data-side trigger the registry omits is invisible — there is no live source to fall back on. |
| **App side** (`src/`) | **Maintained cache, not authority.** Should be accurate and is corrected whenever a gap is found — but need not be provably exhaustive. | The subagent reads live app source on **every** Session 1 review. It can re-derive app-side consumers on the spot, so a stale app-side list is a self-healing miss, not a blind spot — provided the mandate obliges it to look. |

**So the fix is a standing duty, not a bigger grep.** §4.3's Part 3 gains an explicit obligation: on every review, for each registry entry whose data shape or stat key the planned change reads or writes, **re-verify that entry's app-side consumers against live `src/` source** and flag any consumer the entry does not cover — treating the app-side trigger list as a cache to be refreshed, not as the authority.

This is what lets the registry legitimately call itself the reviewer's authority. The claim it makes is precise: **data-side authoritative, app-side re-verified live.** It never claimed, and no longer needs to claim, static app-side exhaustiveness.

> **Supersession.** This reframe supersedes any earlier language in this plan demanding provably-exhaustive app-side triggers — including the phrasing "an entry's triggers are complete only when a change to any real consumer/producer would match" as applied to the app side, and the closing guarantee at the end of §2.2. Data-side completeness is unchanged and remains a hard requirement. The app-side triggers recorded below are still the best current list, and the corrections in this pass still land; they are simply no longer load-bearing on their own.
...
| CR-13 | **Added** `src/utils/outlookPositionStats.js:51/141/153` (team air-yards denominator, AY share, aDOT cell) | **Added** the `aggregateWeeks` loop. Recorded: `rec_air_yd` also has zero literal data-side occurrences |
...
- **Data side: frozen-complete.** Every entry's data-side triggers match a change to any live consumer or producer in `lib/`, `scripts/` or `bin/`, verified by grep against live source rather than inherited from prose. This is the load-bearing claim, because the app-repo subagent has no other way to see the sibling tree. It stays true via the both-repos-same-change rule, not via re-grepping.
- **App side: accurate now, self-healing later.** The app-side triggers are the best current list and three rounds of corrections have landed in them — but they are a **cache**, and the mandate's standing re-verification duty (§4.3 Part 3) is what makes a future gap surface at review time instead of rotting. Do **not** read this section as a promise of static app-side exhaustiveness; §2.2.0 supersedes any such reading.

Re-run the data-side half of this sweep whenever an entry is added or a data-repo family gains a consumer — that is the half that rots silently.
...
- **The two sides carry different completeness burdens.** Each repo's reviewer can read its own tree and cannot read the other one, so:
  ...
  - **The near side of `‖` is a maintained cache.** The reviewer re-verifies it against live source on every review (see the plan-reviewer mandate) and flags consumers the entry does not list. So the near-side list should be accurate, but it is not required to be provably exhaustive, and a gap in it is self-healing rather than silent.
...
Everything between the two sentinel comments below is the **mirrored region**: it is byte-identical to the corresponding region of `sleeper-dashboard-data/README.md`, and the drift check at the end of this file diffs exactly that span. Rules:
...
- **Everything repo-specific goes outside the sentinels**: section framing, the pointer prose, the two `> *Note:*` non-entry paragraphs the data repo preserves, and each file's own terminating heading. Inside the sentinels there is nothing but entries.
- Adding, editing or retiring an entry means editing inside the sentinels in **both** repos in the same change.
...
- **Direction:** both
- **Triggers:** `MAX_SUPPORTED_SCHEMA` in `src/api/dataStore.js`; `isTeamAggregateId` in `src/utils/teamContext.js`; the per-season-`team` readers `resolvePlayerTeam` (`src/utils/playerTeam.js:53-63`) and `resolveAttributedTeam` (`src/utils/teamContext.js:18`, consumed at `:164`/`:195`/`:247`/`:281`, `src/utils/teamRzShare.js:85`, `src/utils/seasonProjection.js:488`); the cross-row summers `computeHistoricalShares` (`src/utils/teamContext.js:161`/`:192`), `computeHistoricalTeamTotals` (`:242-246`), `buildTeamShareTotals` (`src/utils/outlookPositionStats.js:38-40`), `computeEmpiricalAgeCurves` (`src/utils/dynastyScore.js:63-64`) and `buildSeasonPositionRanks` (`src/utils/seasonRanks.js:20`)  ‖  `aggregateWeeks` in `lib/sleeper.mjs`, `scripts/update-nfl.mjs`, `validateNflSeason` in `lib/validate.mjs`, `isTeamAggregateId` in `lib/backtest.mjs`, `buildTeamTotalsForSeason` in `lib/panel.mjs`
...
- **Data side:** `manifest.json`, `lib/manifest.mjs` (`readManifest:19`, `updateManifestEntry:34`) — all 14 `scripts/update-*.mjs` writers register through `updateManifestEntry`
...
- **App side:** `src/api/cfbd.js` `pivotStatRows:85`, `src/api/dataStore.js` `isValidCFBDRows:107` (gates on `playerId` / `statType`), `src/utils/collegeMatch.js:125-127` (pivots all three categories), `src/utils/collegeMetrics.js:69-124` (reads the literals `YDS`, `TD`, `ATT` — dominator rating and the QB score), `src/components/PlayersTab.jsx:681-683` (reads `PCT`, and `COMPLETIONS` as its fallback, for the Player Profile college stat line; also `YDS`/`TD`/`INT` at `:678-680`)
...
- **Direction:** both
- **Triggers:** `pivotStatRows` in `src/api/cfbd.js`, `isValidCFBDRows` in `src/api/dataStore.js`, `src/utils/collegeMatch.js`, the `YDS`/`TD`/`ATT` reads in `src/utils/collegeMetrics.js`, the `PCT`/`COMPLETIONS` reads in `src/components/PlayersTab.jsx`  ‖  `scripts/update-cfbd.mjs`, `lib/cfbd.mjs`, `validateCfbdCategory` in `lib/validate.mjs`
...
- **App side:** `src/utils/seasonProjection.js:445`/`:453` — reads `rec_air_yd` and `rec_tgt` to compute the capture-only `factors.adot` (WR/TE); `src/utils/outlookPositionStats.js:51` (per-season-team air-yards denominator), `:141` (AY share), `:153` (the aDOT cell)
...
| V7 | 18 entries, contiguous ids | `sed -n '/^<!-- CR-REGISTRY-BEGIN -->$/,/^<!-- CR-REGISTRY-END -->$/p' docs/cross-repo-registry.md \| grep -n "^#### CR-"` | `CR-01`…`CR-18`, in order, no gaps — **18 hits** |
| V8 | Every entry has all six fields | for each of `App side` / `Data side` / `Invariant` / `Direction` / `Triggers` / `Mirror`: `sed -n '/^<!-- CR-REGISTRY-BEGIN -->$/,/^<!-- CR-REGISTRY-END -->$/p' docs/cross-repo-registry.md \| grep -c "^\- \*\*<Field>:\*\*"` | `18` each |
...
**Why V7/V8 slice between the sentinels rather than grepping the whole file.** The `#### Entry format` block near the top of `docs/cross-repo-registry.md` is a literal template: it contains one `#### CR-NN · <short contract name>` line and one line for each of the six field labels. A naive whole-file `grep -c` would return 19, not 18, and the discrepancy would look like a missing entry. The template sits **above** `<!-- CR-REGISTRY-BEGIN -->`, so slicing between the sentinels excludes it and every expected count is exactly the entry count. V7 and V8 use the identical slice — the same one the drift check uses — so there is one convention in the whole plan, not two. V6 guards the slice itself: if the sentinels are missing or unbalanced, V7/V8 silently return 0 and the drift check silently returns nothing.
...
Exactly §3.2 of this file — reproduced in the data repo's `README.md` as the `## Entry format` block from Edit E, **verbatim**, including the bullets that follow it. It is already perspective-neutral: `App side` / `Data side` mean the same thing read from either repo, so no field needs adapting. That was the design constraint, and it is met — **the registry body is byte-identical in both repos.**
...
- **The §2.2 sweep's added *data-side* triggers are not individually optional.** They are frozen authority — the app-repo subagent has no live source to fall back on for the sibling tree, so dropping one re-creates a genuinely invisible gap. The added **app-side** triggers should also stay (they are correct, and three rounds of review paid for them), but per §2.2.0 they are a cache rather than the guarantee: a future app-side gap is caught by the reviewer's standing re-verification duty, not by this list being perfect.
- **Chasing static app-side exhaustiveness is explicitly out of scope, by design.** §2.2.0 replaces that goal with the `registry-stale` duty in the mandate. A fifth manual sweep of `src/` is not the fix and should not be scheduled.
