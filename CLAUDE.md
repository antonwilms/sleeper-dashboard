...
| `dp/SeriesBars.jsx` | dp-v2 Slice 1 — arbitrary-length sibling to `CareerBars`; never pads, never substitutes `0` for a null. `'scaled'` (min–max, optional explicit `domain`) or `'signed'` (real zero axis, positives above/negatives below a 1px rule) modes. Ships unused still — Distribution's histogram (dp-v2 Slice 4a) needed a real-zero-count void-slot rule (a bucket count of `0` is itself a void slot, not merely a null entry) that doesn't fit either mode, so it draws its own bars rather than reusing this |
