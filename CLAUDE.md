...
| `dp/PlayerDetailModal.jsx` | 1b Slice ii, **body-only since Slice v; continuous scroll + section index since dp-v2 Slice 3; Game log + Distribution since dp-v2 Slice 4a; Usage & efficiency + Availability & role since dp-v2 Slice 4b** — a non-scrolling row (`min-h-0`, mandatory: without it the row grows to content height under the panel's `overflow-hidden` and the inner scroll column never overflows) of `dp/SectionIndex.jsx` beside a single `overflow-y-auto` scroll column holding **seven** sections, in JSX order (`§overview` — identity row/tiles/chart **and the right rail**, `§game-log`, `§distribution`, `§usage`, `§availability`, `§drivers`, `§why-next`); reads `usePlayerProfile` + the App-level `ProfileDataContext.Provider`, incl. the two `…ByYear` keys `§game-log`/`§distribution` read; dark-only `--color-dp-*`. `SECTIONS` (ids/labels) is a module-level const read by the section wrappers **and must be kept in this same order** — it drives the index and the `IntersectionObserver` scroll-spy, but the rendered order is the JSX order below it, so the two are edited together, never independently; a `useMemo` decorates it per-player with Overview's coverage count (`dynastyScore.signals.seasonsOfData`, reused rather than a fourth "seasons of data" definition) for the index. Active-section highlight is `IntersectionObserver`-driven (root = the scroll column, `rootMargin` shrunk to a top band so a tall section like Overview doesn't outstay its welcome); clicking an index row also calls `scrollIntoView`. The right rail lost its own `overflow-y-auto` (dead once scoped inside a scrolling section) and is `w-full dpwide:w-[300px]` — stacks below `§overview`'s content under 1180px, per the `dpwide:`/`max-dpwide:` variants (never an interpolated `max-[1180px]:` — Tailwind v4 scans class names as literals and would emit no CSS). Section labels render in the index only; the existing card titles remain the sole visible headings. The `!dynastyScore` empty-state branch is unchanged and renders no index, no scroll column. Takes `{ playerId, myTeamName, onCompare }` — no `onClose` (nothing in the body calls it once the scrim/panel/close-× moved to the shell). Rendered by `dp/PlayerDetailTabs.jsx` for whichever tab is active. `§game-log`/`§distribution` are `dp/GameLogSection.jsx`/`dp/DistributionSection.jsx`, fed `careerStats`, `gameLogsByYear[mostRecentSeason]`, `nflScheduleByYear[mostRecentSeason]`, and the already-computed `consistency`. `§usage`/`§availability` are `dp/UsageEfficiencySection.jsx`/`dp/AvailabilityRoleSection.jsx` (own rows below), fed `careerStats`, `playersMap`, and a shared `axisSeasons` (`careerHistory.slice(-5)`'s seasons — computed once here so both sections' per-season rows align, dp-v2 Slice 4b §3.2a); `§availability` also reads the hook's `teamDepthChart`. New leaf module `src/utils/dynastySignalBadges.js` (extracted from `PlayersTab.jsx`'s inline badge block) derives its SIGNALS-rail content; Market's Outlook column set is the helper's second consumer, re-applying `OutlookTab`'s `0.95–1.05` age-curve gate locally rather than editing the shared helper |
| `dp/UsageEfficiencySection.jsx` | dp-v2 Slice 4b — the pop-up's `§usage` body. Renders EXISTING derivations only — `outlookPositionStats.buildPositionStatSeries` (per-position metric set: QB `cmpPct`/`passerRating`/`sacks`; RB `rushShare`/`rbTargetShare`/`yardsPerCarry`; WR/TE `targetShare`/`airYardsShare`/`aDOT`) plus `outlookUsage.buildUsageHistory`'s `snapPct` for RB/WR/TE only (QB is deliberately excluded, not widened — `usageMetrics.js`'s `SNAP_POSITIONS` reasoning). Recomputes `buildTeamShareTotals`/`buildPerSeasonTeamShares` over the whole `careerStats` corpus itself (no `App.jsx` change — `outlookPositionStats.js`/`outlookUsage.js` are a hard zero-diff for this slice; `Market.jsx` already does the same whole-corpus derivation independently, an accepted duplication). Every metric is projected onto the caller-supplied `axisSeasons` (`src/utils/usageEfficiency.js`'s `alignToAxis`/`buildMetricRow`) so every row shares one x-axis and a gap season is a `null` void slot, never a collapsed/absent bar. `SeriesBars` in `scaled` mode with an explicit `[0,1]`/`[0,100]`/`[0,158.3]` domain for bounded fraction metrics (a share series is unreadable auto-scaled); unbounded count/ratio metrics (`sacks`, `yardsPerCarry`, `aDOT`) stay on auto min–max, since a guessed ceiling risks clipping a real outlier. `EPA per opportunity` and red-zone share are both cut, not deferred (task file §3.1–§3.2 — the former needs 4c's multi-season gamelogs load; the latter's only denominator routes both bias older seasons or need `App.jsx` threading). `DISPLAY ONLY` badge on the section |
...
| File | Responsibility |
|------|----------------|
| `ProfileDataContext.jsx` | Provides `{careerStats, playersMap, playerRows, positionPeakPPG, ktcMap, historicalShares, collegeStats, seasonProjections, enrichmentMap, advStats, teamContextByYear, gameLogsByYear, nflScheduleByYear}` (thirteen keys — dp-v2 Slice 2 added the three `…ByYear` keys, view-only and dataSeason-keyed. `gameLogsByYear`/`nflScheduleByYear` gained their first rendering consumers in dp-v2 Slice 4a, `dp/GameLogSection.jsx`/`dp/DistributionSection.jsx`; `teamContextByYear` remains unconsumed — 4b (Usage & efficiency, Availability & role) turned out to need no `App.jsx`-level data at all, so it's now reserved for 4c's Environment section, which needs a multi-season load this single-season key doesn't cover). **One provider site** (down from three as of 1b Slice viii, which retired the Explorer's two `/players`-scoped sites with the surface): an App-level provider wrapping `<Routes>` in `App.jsx`, feeding `dp/PlayerDetailTabs.jsx` and, through it, `dp/PlayerDetailModal.jsx` (`playerRows` key is `playerRowsWithProj`) |
...
| `usageEfficiency.js` | dp-v2 Slice 4b — pure helpers for `dp/UsageEfficiencySection.jsx`: `METRIC_META` (per-metric label/domain/formatters/field-expression/note), `alignToAxis` (projects a sparse `[{season,value}]` series onto a shared flat axis, gap → `null`), `buildMetricRow` (latest real value + signed delta vs. the axis's first season specifically, not latest-vs-prior + coverage count). View-only; no React |
