...
| `/portfolio` | My Team | Rebuilt as a lineup-first screen in Portfolio Slice B — props: `playerRows`, `loaded`, `rosterTeams`, `seasonProjections`, `myTeamName`, `onOpenPlayerDetail`, `tradedPicks`, `ktcPickTable`, `firstLiveDraftSeason`, `draftRounds`, `careerStats`, `playerMap`, `rosterPositions`, `scoringSettings`, `leagueName`, `username` |
