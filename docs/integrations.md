...
**Fantasy point calculation:** Score per-week stats with `calculateFantasyPoints(weekStats, scoringSettings)`. A season row's `fantasyPoints` comes only from `rescoreSeasonTotals` (`src/api/sleeperStats.js`, season-rescore.md): served rows carry Sleeper's half-PPR total, and every `careerStats` season plus the live season's rows are rescored on read from `stats` × the league's `scoringSettings` (the cache keeps the served rows; rescored rows carry `scoringBasis: 'league'`, `sourceFantasyPoints`, `sourceScoringBasis`, and `weeklyPoints` scaled by the season ratio). Season-level dot-products are valid only because scoring keys are per-game counts; `scoreSeasonStats` excludes `NON_ADDITIVE_KEYS` so a rate key can never be summed in. In a season where no row carries a `bonus_fd_*` key, `withFirstDownBonus` derives it from `pass_fd + rec_fd + rush_fd` under the player's current position.
...
**Schema versions:** Manifest entries for `nfl/season-totals/<year>.json` ship at `schemaVersion: 4`. `dataStore.js` advertises `MAX_SUPPORTED_SCHEMA = 4`, the ceiling F-24's stat-key prune raised; this ceiling applies to every family read through `tryDataStore`, not just season-totals. The `isValidSeasonTotals` shape validator only requires v1 fields, so a season that is still on an older version in the data store keeps loading with a blank `availability` shape for that season. (The GP/DNP sparkline this once degraded gracefully in — `AvailabilityHistory.jsx` — was deleted with the Explorer in 1b Slice viii; the `weeklyStatus`/`availability` fields themselves are untouched, just currently unrendered.)
...
### `src/api/teamContext.js` — nflverse team context (view-only)

- **Source:** `${VITE_DATA_STORE_URL}/nflverse/teamcontext/<year>.json` via `tryDataStore`/`getManifestEntry` in `dataStore.js`. `sleeper-dashboard-data` derives it server-side from nflverse pbp (`scripts/update-teamcontext.mjs`) and publishes one file per season as JSON via jsDelivr. Live on the CDN for **2012–2025** (all 14 seasons present in the manifest).
- No API key, no auth.
- **Served shape (re-asserted, never redefined):** `{ schemaVersion: 1, season, generatedAt, rowCount, teamCount, teams }`. `teams` is the **first TEAM-keyed family** — keyed by **era-accurate** team abbr (e.g. `STL`/`SD`/`OAK` for old seasons, not `LA`/`LAC`/`LV`) → `{ games[] }`; each game is `{ week, seasonType, gameId, opponent, off:{…31 fields}, def:{…18 fields} }`. Row identity is `(team, week)`, **not** `sleeper_id`. `off` fields: `plays, passPlays, rushPlays, passRate, epaSum, epaPlays, epaPerPlay, passEpaSum, passEpaPlays, passEpaPerPlay, rushEpaSum, rushEpaPlays, rushEpaPerPlay, successes, successPlays, successRate, proePlays, proePassPlays, proeXpassSum, proe, rzTrips, rzPlays, rzPassPlays, rzRushPlays, rzPassRate, rzTdTrips, rzFgTrips, neutralSeconds, neutralGaps, neutralSecPerPlay, pointsScored`. `def` fields: `plays, passPlays, rushPlays, epaSum, epaPlays, epaPerPlay, passEpaSum, passEpaPlays, passEpaPerPlay, rushEpaSum, rushEpaPlays, rushEpaPerPlay, successes, successPlays, successRate, rzTripsAllowed, rzTdTripsAllowed, pointsAllowed`. See the data repo's `data-catalog.md` for the authoritative field list.
- **Week numbering** is continuous REG→POST and unique within a team-season (e.g. a wild-card bye skips a week number; `rowCount` includes POST rows).
- **Single-week-rate rule:** per-week rates (`proe`, `epaPerPlay`, `successRate`, `rzPassRate`, `neutralSecPerPlay`, `passRate`, …) are single-game values — **never sum or average them across weeks**; the rows ship the counting components (`epaSum`/`epaPlays`, `proeXpassSum`/`proePassPlays`, `neutralSeconds`/`neutralGaps`, …) precisely so a consumer aggregates components and re-divides. The loader is pass-through and transforms nothing.
- **Team-code domain:** era-accurate, the same domain as the NFL schedule and the season-totals per-season `team` field. A consumer resolving a player's team for a join uses `src/utils/playerTeam.js` (`resolvePlayerTeam`), which era-remaps the current-franchise gamelogs domain but is an identity transform on the already-era-accurate season-totals domain.
- **Explicit-season signature** — `loadTeamContext(year)` takes one explicit season; it does **not** probe down. A teamcontext consumer needs an arbitrary past season, so a "most-recent-available" probe is the wrong shape here — identical reasoning to `nflSchedule.js`/`nflGameLogs.js`. A caller wanting the current season resolves it from `nflState.season`: `loadTeamContext(parseInt(nflState.season, 10))`.
- **Cache:** `nfl-teamcontext/<year>` per year, permanent TTL (999999 min). Each record stores `{ teams, season, rowCount, lastModified }`. Past seasons are immutable so the permanent cache is correct; the current season (2025) mutates weekly, and the manifest `lastModified` freshness check re-fetches it.
- **`MIN_TEAMCONTEXT_ROWS = 60`** sparsity floor — a shared cross-repo constant (both repos change together), defined and exported from `dataStore.js`. Enforced three times: `isValidTeamContext` rejects a below-floor file at the `tryDataStore` boundary, the cache-hit guard re-checks the floor on the IndexedDB path, and `loadTeamContext` re-asserts it on the file's declared `rowCount` after fetch.
...
**`projectionBasis` (season-rescore.md, still `schemaVersion: 3`):** a top-level envelope field naming the basis every `careerStats` season was scored on — `'league'` (each season rescored through `rescoreSeasonTotals` with this snapshot's own `scoringSettings`), `'half_ppr'` (served basis), `'mixed'`, or `'unknown'` (no careerStats). **An absent field means a pre-switch capture whose projections are half-PPR**, even though its `scoringBasis` label may read `'custom'` — except a season whose `inputStatus.careerStats.detail.provenance` is `'live-api'`, which was league-scored before the switch too. Additive: no reader breaks on the key, so there is no version bump.
