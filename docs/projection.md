...
Steps 5, 5c, 5d, 5e, 5f, 5g and 7b feed `combinedNewFactor = clamp(momentumFactor × qbQualityFactor × breakoutFactor × bounceBackFactor × tdRelianceFactor × trajectoryFactor × efficiencyFactor × snapShareFactor × rzUsageFactor, 0.78, 1.30)` — a cap on the nine new PPG multipliers. Post-D2 the natural range is [0.607, 1.685], so the clamp binds for a meaningful tail of stackers. It is a deliberate ceiling, not a never-bind guardrail; future batches add factors inside it rather than widening it. (Trend note: at nine multiplicands the outer clamp is shading from guardrail toward per-signal cap — one more inner factor should trigger a restructure into a single normalized aggregate index rather than another raw-product add.)
