...
| NFL per-season team (`team`) | raw ingested data | data: `nfl/season-totals/<year>.json` (schema v3) ← Sleeper weekly `team` | **2012–present** | **Reconstructable** — Sleeper weekly source; backfillable per season | **scoring-load-bearing since the R2 flip (2026-07-11)** — feeds projection Steps 3/5h attribution (`careerStats[season][pid].team` via `resolveAttributedTeam`); also view-only display (NFL-stats game-log schedule join — `NflStatsTab`; Outlook share attribution — `OutlookTab`); dynasty-score channels remain current-team-pinned; resolution for view surfaces via `playerTeam.resolvePlayerTeam` |
