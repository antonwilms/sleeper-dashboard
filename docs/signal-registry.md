...
| Share trend (`shareTrend`, `shareTrendRaw`, `shareVolatility*`) | computed factor | `teamContext.js` / `seasonProjection.js` Step 3 | 2012+ | Reconstructable | active→projectedPPG (neutralized 1.0 on team-change); attribution per-season-team since the R2 flip (2026-07-11); dynasty boost consumes a current-team-pinned copy |
