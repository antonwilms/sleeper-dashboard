...
- **Efficiency** (dp-v2 Slice 5b, the fourth set, joining Volume in the `ON FIELD` group) — per-position (`EFFICIENCY_COLUMNS` in `market/columnDescriptors.js`; `ALL` reuses WR/TE's list rather than inventing a fifth cross-position shape): QB `EPA/ATT`/`CPOE`/`SACK%`/`AY/ATT`/`RUSH EPA`; RB `CARRY SH`/`TGT SH`/`RUSH EPA/ATT`/`YAC`/`BTKL`; WR/TE `TGT SH`/`AY SH`/`aDOT`/`EPA/TGT`/`RACR`/`RZ SH`/`SNAP%`/`DROPS`. Single-season, **pinned to `dataSeason`** — the season `<select>` stays hidden, unlike Volume's; the header states this explicitly ("Fixed to the *N* season … unlike Volume") rather than leaving the numbers' disagreement with Volume unexplained.
  ...
  - **Denominator floors on the four per-opportunity rates** (`EPA/ATT`, `CPOE`, `RUSH EPA/ATT`, `EPA/TGT`) — `seasonEfficiency.js`'s exported `MIN_PASS_ATTEMPTS = 100` / `MIN_CARRIES = 25` / `MIN_TARGETS = 25` null out a rate below its floor rather than emit it at a denominator of 1; the render path already turns `null` into `—`, so a floored player simply sinks to the bottom of a descending sort via `compareNullsLast` (a 3-attempt backup at CPOE +29pp no longer outranks a 600-attempt starter at +2.2pp). **Deliberately not applied to the shares** (`CARRY SH`/`TGT SH`/`AY SH`/`RZ SH`/`SNAP%`) or to `RUSH EPA` (`rushEpaTotal`, a season total) — a share at a small denominator is a true small share, not a noisy point estimate; only the four rates get the gate. The three floor values differ because they belong to the denominator, not the set — a single shared floor would blank most WRs at the QB floor.
  - **`RACR`** reads the `advStats` prop directly (`advStats?.byId?.[id]?.racr`, gated on `advStats.complete` **and** `advStats.year === dataSeason` — a result loaded for a different season never renders under the pinned header) rather than re-deriving `receivingYards÷receivingAirYards` from gamelogs — Market is advstats' first UI consumer since the Explorer's `AdvancedStatsPanel.jsx` was deleted in 1b Slice viii.
