...
dp-v2 Slice 6a — the 32-team index (`/teams`, under `MANAGE` after Market in both the rail and the mobile tab bar). Slice 6 split on the data-dependency seam: this half needs **zero new fetching** — it reads `teamContextByYear[dataSeason]` (already loaded across a five-season window since Slice 4c) plus `playerRows`. **Rows became clickable in dp-v2 Slice 6b**, which added team detail (`/teams/:abbr` — see its own section below) — whole-row + keyboard, navigating via a plain `window.location.hash` write rather than `useNavigate()`/`<Link>` (this component stays Router-context-free, so `teams/Teams.test.jsx`'s existing un-wrapped renders keep passing).
