...
**Segmented column-set control** — Value / Outlook / Production, persisted to `localStorage['market-column-set']`, validated on read. Position pills (All/QB/RB/WR/TE) sit below it, in the same row as the filter bar (1b Slice vi). Saved presets and free-text search are still deferred (slice vii).
...
- **Eleven dimensions**, grouped Player (starters-only, rookies-only, age range, experience range) / Availability (all/my roster/available/NFL free agent) / Team (NFL team + fantasy team multi-selects) / Dynasty (four dynasty-group chips, market-signal radio, KTC range) / Projection (min projected games, new — default `0`, i.e. off).
