...
The redesign's data-display surface (1b Slice iii, `.claude/tasks/dynasty-portfolio-1b-iii-market.md`) — one table over `playerRowsWithProj`, dark-only `--color-dp-*`, no `ProfileDataContext` (gets `playerRows`/`loaded`/`careerStats`/`playerMap`/`seasonProjections`/`ktcHistory`/`myTeamName`/`onOpenPlayerDetail` as props from `App.jsx`; `ktcHistory` added dp-v2 Slice 5a, for the TREND gutter's sparkline only). Row click (or Enter/Space on a focused row — rows are `role="button"`/`tabIndex={0}`) calls `onOpenPlayerDetail(player_id)`, opening the Slice ii detail pop-up. `dp/MarketTable.jsx` is the presentational shell (`PAGE_SIZE = 50`, own position-pill row and pager). Its cell components (`SortTh`, `PlayerCell`, `ClickableRow`, `CareerBars`, `DeltaCell`) moved to `dp/cells.jsx` in Slice iv so Portfolio could import rather than fork them — a pure relocation, Market's own rendering unchanged.
...
4. The `TREND` gutter's `_trend` key is a comparator branch in every set (reading `.delta` off an object), not a bare-key lookup — `Value`/`Outlook` resolve `a[key]` off the row and `Volume` resolves `a._avg?.[key]`, neither of which reaches a nested delta, so all three sort branches special-case `_trend` the same way `_snapTrend`/`_oppTrend` already did.
