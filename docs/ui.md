...
dp-v2 Slice 6a — the 32-team index (`/teams`, under `MANAGE` after Market in both the rail and the mobile tab bar). Slice 6 split on the data-dependency seam: this half needs **zero new fetching** — it reads `teamContextByYear[dataSeason]` (already loaded across a five-season window since Slice 4c) plus `playerRows`. Team detail (`/teams/:abbr`, the 14-season charts/holdings/coaching screen) is 6b, a separate slice; **rows here are not clickable** — this program ships no dead links or placeholder routes (`ACT` was removed in 5a for exactly that reason).
...
**Distribution strip** — beneath the header, `dp/SeriesBars.jsx` reused unmodified (a 32-team distribution is exactly its shape: an arbitrary-length number array, no time axis implied), redrawn for whichever column is currently sorted, in the same row order as the table.
