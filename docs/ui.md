...
- **In-season** (in-season-evidence-1-view.md, in `MODEL & MARKET` after Outlook) — nine columns: `PLAYER` · `TREND` · `G <live>` · `PPG <live>` · `CURRENT PROJ` · `ROS` (the projection updated with this season's games, then the share of the estimate that is this season; **ext** marks rookies and players under 8 games last season, whose weights are extrapolated) · `OPP/G <prior>` · `OPP/G <live>` · `OPP SHIFT` (signed, or the season's rate with a **new role** marker when there was no real role last season). View-only: computed by `utils/inSeasonEvidence.js` from the `currentSeasonTotals` prop only while the set is active, and never reaches `playerRows`, the projection or a snapshot. Renders `—` (and a header note saying so) when no usable in-progress file is loaded; the note reads "Scored on this league's settings." when both the prior season and the live rows went through the rescoring seam, else the Half-PPR basis line. The dynasty-horizon posterior is computed but not rendered.
...
- **Efficiency** (dp-v2 Slice 5b, the fourth set, joining Volume in the `ON FIELD` group) — per-position (`EFFICIENCY_COLUMNS` in `market/columnDescriptors.js`; `ALL` reuses WR/TE's list rather than inventing a fifth cross-position shape): QB `EPA/ATT`/`CPOE`/`SACK%`/`AY/ATT`/`RUSH EPA`; RB `CARRY SH`/`TGT SH`/`RUSH EPA/ATT`/`YAC`/`BTKL`; WR/TE `TGT SH`/`AY SH`/`aDOT`/`EPA/TGT`/`RACR`/`RZ SH`/`SNAP%`/`DROPS`. Single-season, **pinned to `dataSeason`** — the season `<select>` stays hidden, unlike Volume's; the header states this explicitly ("Fixed to the *N* season … unlike Volume") rather than leaving the numbers' disagreement with Volume unexplained.
  ...
  - **Denominator floors on five per-opportunity rates** (`EPA/ATT`, `CPOE`, `RUSH EPA/ATT`, `EPA/TGT`, `RACR`) — `seasonEfficiency.js`'s exported `MIN_PASS_ATTEMPTS = 100` / `MIN_CARRIES = 25` / `MIN_TARGETS = 25` null out a rate below its floor rather than emit it at a denominator of 1; the render path already turns `null` into `—`, so a floored player simply sinks to the bottom of a descending sort via `compareNullsLast` (a 3-attempt backup at CPOE +29pp no longer outranks a 600-attempt starter at +2.2pp). `RACR` shares `MIN_TARGETS` with `EPA/TGT`, but applies it to the advstats row's own `components.targets` via `src/utils/liveAdvStats.js`'s `flooredRacr`, not inside `seasonEfficiency.js` — its denominator comes from a different served family. **Deliberately not applied to the shares** (`CARRY SH`/`TGT SH`/`AY SH`/`RZ SH`/`SNAP%`) or to `RUSH EPA` (`rushEpaTotal`, a season total) — a share at a small denominator is a true small share, not a noisy point estimate; only the five rates get the gate. The floor values differ because they belong to the denominator, not the set — a single shared floor would blank most WRs at the QB floor.
  - **`RACR`** reads the `advStats` prop directly (`flooredRacr(advRow)`, gated on `advStats.complete` **and** `advStats.year === dataSeason` — a result loaded for a different season never renders under the pinned header) rather than re-deriving `receivingYards÷receivingAirYards` from gamelogs — Market is advstats' first UI consumer since the Explorer's `AdvancedStatsPanel.jsx` was deleted in 1b Slice viii. Blank below `MIN_TARGETS` targets.
  - **`RACR <liveSeason>`** (advstats-live-season-column) sits beside the completed `RACR` column, WR/TE only, sourced from `loadAdvStatsForSeason`'s exact-year, no-fallback load (`src/utils/liveAdvStats.js`'s `usableLiveAdvStats`/`liveRacrCell`). Shown only when the load resolved for exactly `liveSeason` and `liveSeason > dataSeason`; each qualifying cell needs `components.targets ≥ MIN_TARGETS` (25, the `EPA/TGT` floor reused rather than a second floor) and a finite integer `components.weeks ≥ 1`, and renders as `` `<racr> · <weeks> wk(s)` `` — a value is never shown without its weeks. The column is **hidden**, not shown as all dashes, when no usable live file exists (offseason, a live file below the row floor, or a failed load); a `market-sort` of `racrLive` restored while the column is hidden falls back to the position's default sort, but only once the live load has settled (`advStatsLive != null`) — while it is still pending, a persisted `racrLive` sort is left alone.
...
## Teams (`src/components/teams/Teams.jsx`)

dp-v2 Slice 6a — the 32-team index (`/teams`, under `MANAGE` after Market in both the rail and the mobile tab bar). Slice 6 split on the data-dependency seam: this half needs **zero new fetching** — it reads `teamContextByYear[dataSeason]` (already loaded across a five-season window since Slice 4c) plus `playerRows`. **Rows became clickable in dp-v2 Slice 6b**, which added team detail (`/teams/:abbr` — see its own section below) — whole-row + keyboard. **6b navigated via a plain `window.location.hash` write to avoid wrapping `teams/Teams.test.jsx`'s renders in a Router; dp-v2 Slice 7 §8 reversed that** (tests should follow production shape, not set it, and a raw hash write is invisible to the router) — navigation is now `useNavigate()` via a component-scoped callback, and the test file's 14 renders are wrapped in `MemoryRouter` instead.

**Columns**, in order — `TEAM · PROE ↓ · PACE · SUCC% · OFF EPA/PL · RZ TD% · DEF EPA ALL · PTS/G · YOUR EXPOSURE`, default sort PROE descending. Every column except `TEAM` and `YOUR EXPOSURE` comes straight off `src/utils/environment.js`'s `computeTeamSeasonMetrics` via the new `buildTeamMetricsTable(loaded)` export — one pass over all 32 teams, an additive sibling to `buildLeagueRankTable` (5c) that returns metric **values** rather than ranks, since this index renders and sorts on values and consumes no ranks at all. **The 32 rows are driven by the loaded season's own `teams` keys** (era-accurate domain — `LA`, `LAC`, `LV`), not `marketFilters.NFL_TEAMS` (the Sleeper domain, which carries `LAR`) — so the row set and the exposure join below share one domain by construction.

**`DEF EPA ALL` is the one inverted column.** `usePlayersTable`'s `handleSort` hard-codes which columns sort ascending on their first click (`full_name`, `ceilingRank`, `floorRank`); this slice added `defEpaPerPlay` to that set, since it's a lower-is-better number and the first click should surface the best defences. Colour is inverted too — **negative renders blue (up), positive renders amber (down)**, the reverse of `OFF EPA/PL`'s rule. Neither piece touches `environment.js`'s `LOWER_IS_BETTER` set (still exactly `{'pace'}`) — that set's only consumers are `computeLeagueStanding`/`buildLeagueRankTable`, and this index calls neither, so extending it would be a no-op with no effect on what's actually rendered here.

**`YOUR EXPOSURE`** — the column the surface exists for. Scoped to rows where `ownerTeamName === myTeamName`, grouped by **`normalizeTeamForSchedule(nfl_team)`**, never raw `nfl_team`. `nfl_team` is `playerMap[id].team` — the **Sleeper** domain (`LAR`) — while the row set above is the **era-accurate** domain (`LA`); ungated, every Rams player lands in an unmatched bucket and the `LA` row silently reads `none` with nothing else on screen looking wrong (`normalizeTeamForSchedule`, `nflStats.js` — CR-16's app-side boundary). `nfl_team` is also the literal string `'FA'` for a free agent, not null — a real asset that belongs in the roster-value **denominator** but has no team bucket, so shares sum to **≤100%** and are **never rescaled to 100%** (rescaling would silently redistribute value the user holds in unrostered players into the teams they *do* have exposure to). A team with nobody owned renders `none` / `—`, muted — not `0 players` / `0%`; a null `ktcValue` is skipped in both the share numerator and denominator, never treated as a measured zero (a bucket whose players are *all* null-valued renders `—` for the share, not a computed `0.0%`, since that would falsely claim a known-zero contribution rather than an unknown one). `myTeamName` null renders the whole column `—` throughout, matching Portfolio's own precedent for a rosterless account. `resolvePlayerTeam` is deliberately **not** used here — exposure is a question about *now* (which NFL team a player is currently on), and that helper answers a historical per-season question.

**Distribution strip** — beneath the header, `dp/SeriesBars.jsx` (a 32-team distribution is exactly its shape: an arbitrary-length number array, no time axis implied), redrawn for whichever column is currently sorted, in the same row order as the table. **Passes `colour="neutral"` since dp-v2 Slice 6b** — `SeriesBars`' `signed` mode otherwise colours by raw sign, which used to contradict `DEF EPA ALL`'s own inverted cell colours right above it in the table (a follow-up 6a itself flagged); `colour="neutral"` is a no-op for the `scaled`-mode columns, which were already neutral.

**No coverage pips** — same reasoning as the table body elsewhere in this family: `computeTeamSeasonMetrics` has no null-coverage story worth pipping here.

**Degraded state** — two distinct states, per the same `DegradedBlock` vocabulary used elsewhere: `loaded===false` (career load in progress, like Market/Portfolio) renders the plain loading line, not a `DegradedBlock` — the data is not missing, it hasn't arrived. `loaded===true` but `teamContextByYear[dataSeason]` absent or `complete!==true` renders one whole-surface `DegradedBlock` (`kind="not-yet-accruing"`, named explicitly — an unrecognised `kind` falls through to `DegradedBlock`'s own empty-label case) — there is nothing else on this screen to show, unlike the pop-up's per-section degradation.
...
Four sortable columns appended after `PTS/G`: **`FPA QB`, `FPA RB`, `FPA WR`, `FPA TE`** — blended per-game fantasy points a defense has allowed to that position group, one decimal, rank 1 = toughest (lowest points allowed). Derivation lives in the new pure util `src/utils/opponentStrength.js` (`FPA_POSITIONS`, `PRIOR_WEIGHT_GAMES`, `FPA_PRIOR_DROP_GAMES`, `computeFpaPerGame`, `buildFpaTable`, `rankFpaTable`) — see that file's own header and its CLAUDE.md row for the math and the join.

**The blend is shrinkage, not a switch:** `fpaPerGame = (gCur·rateCur + K·ratePrior) / (gCur+K)`, `K = PRIOR_WEIGHT_GAMES = 3` pseudo-games. `gCur = 0` (no current-season games, including the week-1 bye case) is guarded explicitly — `computeFpaPerGame` returns `null` for `gamesPlayed <= 0` and the caller drops the term, rather than computing `fpa/0` (`Infinity`, and `0 * Infinity === NaN` in JS). The app *does* have in-progress-season season-totals (`in-season-app-read.md`, CR-21): the live season's row map reaches this module as `currentRows`, so `currentSeason` populates once that season's file exists and a column can render a real blend, not only the last-completed-season rate. `K = 3` is derived from measured year-over-year stability of points-allowed; the study is not reproduced in-repo. Once a defense has played `FPA_PRIOR_DROP_GAMES` (9) or more current-season games, the prior is dropped entirely and the blend reads 100% current — the current-season term does not merely approach a ceiling, it reaches one.
...
dp-v2 Slice 6b — `/teams/:abbr`, completing Slice 6. Reads `:abbr` itself via `useParams` — `App.jsx` is route-unaware (no `useLocation`/`useNavigate` anywhere in it), and `LeagueView.jsx` is the existing precedent for a route-aware surface reading its own param. `App.jsx` passes data only; it does not know the current team.

**The 14-season window loads on demand.** The index needs one season; only this surface needs the full window (teamcontext has exactly 14 seasons, 2012–2025). At ~1 MB/season that's ~9 MB beyond the five seasons Slice 4c already loads eagerly — too much to pay on every app load for a surface many users never open. `TeamDetail` calls the `onNeedTeamHistory` prop once on mount; `App.jsx` threads it from `src/hooks/useTeamHistoryLoader.js` (see that file's own `src/hooks/` table row), which widens `teamContextByYear` through the *same* year-keyed setter the eager 5-season effect already uses — additive, that effect is untouched. The card renders whatever is loaded and lets `SeriesBars` void-slot the rest; it never gates behind a complete window, and never shows a spinner over a chart that already has real seasons.

**Four metric cards** — `PROE` · `PACE` · `SUCCESS RATE` · `OFF EPA / PLAY`. Each: the current-season value, the league median + rank (`computeLeagueStanding(loaded, metricId, team)`, `Nth of N` — `N` is teams with a **non-null** value for that metric, not guaranteed 32), a percentile bar derived from the real `rank`/`n` (`(n-rank)/(n-1)` — `computeLeagueStanding` returns no p10/p90, so none are fabricated), a 14-season `SeriesBars` chart, a direction label (`VOLUME SIGNAL · NOT A QUALITY READ` for PROE; `LOWER IS BETTER` for pace; `HIGHER IS BETTER` for success rate and off EPA/play), an axis label, and the field expression. `scaled` cards **state their floor** in the axis label (`SUCCESS RATE` → `AXIS 0–100%`, an explicit `[0,1]` domain; `PACE` → `AXIS <lo>–<hi>s`, computed from that team's own 14-season min/max since pace has no natural fixed bound); `signed` cards (`PROE`, `OFF EPA / PLAY`) read `ZERO BASELINE` instead and pass `colour="neutral"` (`SeriesBars`' Slice 6b addition — see its own row) since a metric whose sign isn't a verdict must not editorialise. Every value comes straight off `computeTeamSeasonMetrics` (4c) — none of the four field expressions are re-derived.

**The 14-season lookup resolves every season through `eraTeam(abbr, season)`** (`utils/playerTeam.js`) — the single highest-stakes correctness point in this slice. teamcontext is keyed **era-accurately**, so a franchise's key changes partway through the window: `/teams/LA` looked up as a fixed `'LA'` finds **nothing for 2012–2015** (the real key is `STL`) and renders four void slots as though the data were missing, with nothing else on screen looking broken. Same for `/teams/LAC` (`SD` ≤2016) and `/teams/LV` (`OAK` ≤2019). The route param itself is a **current** code, matching what the Teams index links — a historical code typed directly (`/teams/STL`) is not accepted and renders the unknown-team degraded state below, rather than doubling the surface's identity across two domains.

**Holdings ("Rostered & tracked")** — the skill players `playerRows` carries for this team, explicitly **not** framed as a full depth chart: `playerRows` is relevance-gated (`isRelevantPlayer` drops un-rostered non-rookies with no play in the last two seasons and no KTC row), so this list is real but partial. Filtered on `normalizeTeamForSchedule(nfl_team) === abbr` — the same domain hop the Teams index's `YOUR EXPOSURE` column established, since the route param is a current code — sorted by `ktcValue` descending, nulls last. Per-row meta: the user's own players read `N% of roster` (`dp-up-text`, that player's `ktcValue` ÷ the user's whole roster value — the denominator reuses `teamExposure.js`'s `buildExposure`, not a second sum); another manager's read `owned by <manager>` (muted); unowned read `not owned` (muted). A team with no matching rows renders one muted line, not an empty table and not a `DegradedBlock` — a real, correctly-computed emptiness.

**Coaching** resolves two silent traps, both of which would otherwise render an empty block with nothing looking wrong:
- **Domain.** `enrichment/coaching.json` keys the **Sleeper** domain (`LAR`), while the route param is era-accurate (`LA`) — `getCoaching(payload, 'LA', …)` finds nothing for the Rams. `nflStats.js`'s new `denormalizeTeamForSchedule` (the additive reverse of `normalizeTeamForSchedule`, derived from the same `SCHEDULE_TEAM_ALIAS` constant so the two can't drift) maps the route param back before the lookup — the mirror image of the exact bug 6a caught in its own exposure join.
- **Year.** Every coaching entry is a fixed future year, not `dataSeason` — querying the charted season returns `{HC:null,OC:null,DC:null}` for every team. The surface derives the enrichment's **own** year locally (the max `year` among that team's entries, falling back to the max across all entries) and **labels the block with it**, so the reader knows coaching is a "now" fact, not an attribute of the charted season. `utils/enrichmentLookup.js` stays unedited — a year-resolving helper there would widen a CR-03 definition site for no gain.
...
- **`PTS` reads the display series from `outlookConsistency.resolveDisplayWeeklyPoints`** (object, 1-based key), not `gamelogs`' nflverse `fantasyPoints`/`fantasyPointsPpr` — for a store season the served half-PPR week (`sourceWeeklyPoints`, since `rescoreSeasonTotals` rescales only season totals exactly), labelled half-PPR under the table; for a live-API season the league-scored week, labelled as league scoring; `—` when the row's basis is unknown. It is REG-only, so every POST row's `PTS` is `—` by design.
...
**`§distribution`** (`dp/DistributionSection.jsx`, bucket helpers in `src/utils/distribution.js`) — a histogram of per-game fantasy points, over `computeConsistency`'s **own** `seasons` window (the 3-qualifying-season pool — reused, never re-derived); every value — histogram, mean/SD/CV, counts, ±1 SD markers, pips — is computed over the display series via `extractDisplayGamePoints`/`summarizeGamePoints`, so its SD equals the Overview tile's `±SD` only for league-scored weekly rows; a caption states the basis (or the per-season mix). Buckets are 5-point (`0–5 … 30–35`, `35+`) **plus an explicit `<0` bucket** (per-game league points do go negative). An empty bucket renders a void slot (dashed baseline), never a zero-height fill. A shape block alongside gives pooled mean/SD/CV and `X of N` counts over 20 and under 10, `N` always the pooled game count; coverage pips read the pooled **game** count via `coverageBand`, not the season count (`coverageBand(≤3)` would read `'low'` for every player if seasons were used, since `computeConsistency` caps its window at 3 seasons). `consistency === null` renders `NOT YET — ACCRUING`; a non-null consistency with a null `sd` (pooled games under `MIN_POOLED_GAMES`) still draws the histogram and shape block, with the SD row and the ±1 SD dashed markers omitted.
