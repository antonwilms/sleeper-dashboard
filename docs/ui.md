...
- **`PTS` reads `careerStats[...].weeklyPoints[week]`** (object, 1-based key) — this app's league-scored points, not `gamelogs`' nflverse `fantasyPoints`/`fantasyPointsPpr`; the store has no per-week stats, so each week is the served week scaled by the season's league ÷ half-PPR ratio (the weeks sum to the rescored season line; a per-week value is approximate, `PROVISIONAL(heuristic)`, and `—` when the ratio is undefined). It is REG-only, so every POST row's `PTS` is `—` by design.
