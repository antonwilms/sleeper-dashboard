...
**Body (`PlayerDetailModal.jsx`, 1b Slice ii, body-only since Slice v; continuous scroll + section index since dp-v2 Slice 3)** — a non-scrolling row of a **140px section index** (`dp/SectionIndex.jsx`) beside a single `overflow-y-auto` scroll column holding three sections: `§overview` (identity row, four tiles — Dynasty score / Market value / Next season / Floor risk, values only, `PROVISIONAL(no-data)` on the Market-value tile's omitted 30-day Δ — a Career-PPG-and-projection bar chart, **and the right rail**), `§drivers` ("What drives the score" — the five weighted `dynastyScore.components`), and `§why-next` ("Why next season" — projection adjustment chips + closest career comps). Section labels render **in the index only**; the existing card titles (`What drives the score`/`Why next season`) remain the sections' sole visible headings. The **right rail** (POSITION IN PORTFOLIO share, SIGNALS badges via `src/utils/dynastySignalBadges.js`, RANK THIS SEASON peers) is scoped inside `§overview` and scrolls away with it, rather than spanning the full body height as before Slice 3. Clicking an index row scrolls to the section (`Element.scrollIntoView`) and highlights it; the active section is otherwise tracked via `IntersectionObserver` against the scroll column, worked cleanly (no click-only fallback needed). No route, no hash — the pop-up is deliberately routeless. **At ≥1180px** (the `dpwide:` breakpoint token, `--breakpoint-dpwide` in `src/index.css`'s `@theme`): index + main column + 300px right rail beside `§overview`. **Below 1180px**: the index is hidden and the right rail stacks full width below the `§overview` content (below the career chart, not literally "under the tiles" — see the Slice 3 task file §4 for why). Eight empty states are handled explicitly (null `dynastyScore` entirely — this branch renders no index and no scroll column — null `.components`, null `.signals`, null `projection`, null `ktcValue`, `computeConsistency` returning `null`, a non-null consistency object with a null `sd`, and an empty `comps` list) — see `PlayerDetailModal.test.jsx`. Takes `{ playerId, myTeamName, onCompare }`; has no `onClose` of its own since Slice v moved the close affordances to the shell.
