        ...
        return (
          <div key={team.rosterId}>
            <div className="mb-3 pb-1 border-b">
              ...
              <span className="text-[var(--color-text-faint)] text-sm ml-2">— {team.managerName}</span>
            </div>
            {[...POSITION_ORDER.map(pos => ({ pos, players: grouped[pos] })), { pos: 'Other', players: other }]
              .filter(({ players }) => players.length > 0)
              .map(({ pos, players }) => (
                <div key={pos} className="mb-4">
                  <div className="text-xs font-semibold text-[var(--color-text-faint)] uppercase tracking-wide mb-1">{pos}</div>
                  <div className="space-y-1">
                    {players.map(p => (
                      <div key={p.id} className="flex items-center gap-3 text-sm">
                        <SlotBadge slot={p.slot} />
                        <span className="font-medium w-40 truncate">{p.full_name ?? p.id}</span>
                        <span className="text-[var(--color-text-faint)] w-8">{p.team ?? 'FA'}</span>
                        {p.age != null && <span className="text-[var(--color-text-faint)]">Age {p.age}</span>}
                      </div>
                    ))}
                  </div>
                </div>
